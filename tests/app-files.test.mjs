...
test("includes the requested offline-first features", async () => {
  ...
  assert.match(app, /<circle className="profile-poi-marker"[^>]*aria-hidden="true"/);
  assert.doesNotMatch(app, /selectedProfilePoiId|hoveredProfilePoiId|activeProfilePoi|profile-poi-label/);
  assert.doesNotMatch(app, /onMouseEnter=|onMouseLeave=|onToggle|role="button"[^>]*point of interest/);
  assert.match(styles, /\.profile-poi-marker/);
  assert.match(styles, /\.profile-poi-marker \{[^}]*pointer-events: none/);
  assert.doesNotMatch(styles, /\.profile-poi-(?:dot|hit-area|label)/);
  ...
  assert.match(styles, /\.upcoming-poi-card/);
  assert.match(styles, /\.upcoming-poi-list/);
  assert.match(styles, /\.itinerary-poi-row/);
  assert.match(styles, /\.total-walk-card/);
  assert.match(planning, /export function upcomingPointsOfInterest/);
  assert.doesNotMatch(planning, /export function nextPointOfInterest/);
